...
class Summary:
    ...
    def _persist_running_item(self, client: ContainerProxy):
        ...
        # TODO: Refer same Running const with SDK
        item = SummaryLine(
            id=trace_id,
    ...
    def _insert_evaluation_with_retry(self, client: ContainerProxy):
        for attempt in range(3):
            try:
                ...
                # but init cosmosDB client is time consuming, and the later request will reuse the same client,
                ...
                if attempt == 2:  # If this was the last attempt, ignore and just return
    ...
    def _insert_evaluation(self, client: ContainerProxy):
        ...
        if items:
            if items[0].get("status", "") != "Ok":
                raise InsertEvaluationsRetriableException("Main flow test is still running, cannot patch evaluation.")
            main_id = items[0]["id"]
        else:
            raise InsertEvaluationsRetriableException(
                f"Cannot find main run by parameter {referenced_line_run_id}, {referenced_batch_run_id}, {line_number}"
            )

        # From concept, we should be able to let a batch run reference a line run or vice versa.
        if SpanAttributeFieldName.LINE_RUN_ID in attributes:
            item.line_run_id = attributes[SpanAttributeFieldName.LINE_RUN_ID]
        else:
            item.batch_run_id = attributes[SpanAttributeFieldName.BATCH_RUN_ID]
